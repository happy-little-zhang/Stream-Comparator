...
int main()
{
	...
	//周期性执行定时任务
	Timer t_send_normal;
	t_send_normal.StartTimer(ECU4_SEND_CYCLE, std::bind(can_send_normal));

	Timer t_send_injection;
	//t_send_injection.StartTimer(ECU2_SEND_CYCLE, std::bind(can_send_injection));
